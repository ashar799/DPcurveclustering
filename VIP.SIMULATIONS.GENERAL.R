...
############################################### MAKING X unordered from the clusters data #####################
X.un<- c(0)
for (i in 1:F){
  X.un <- rbind(X.un, data.new[[i]])
} 
X.un<- X.un[-1,]

X <- matrix(NA, nrow = N, ncol =D)
X[order,] <- as.matrix(X.un)

###### True Labels for the points ###################################################################################
c.true.un <- c(0)
for ( i in 1:F){
  c.true.un <- rbind(as.matrix(c.true.un) , as.matrix(c(rep(i, as.integer(N * p.dist[i])))))  
}
c.true.un <- as.factor(c.true.un[-1,])

c.true <- c(0)
c.true[order] <- c.true.un


### A little Visualization of the X Data ##############
pc <- prcomp(X)
...
#### Assuming All points contribute equally
w <- rep(0.5,N)
...
### Generating Y #######################################
Y.rel <- matrix(0, nrow = Np, ncol = rel.D)
for ( i in 1:Np){
  pat <- which(X.info[,1] == i)
  prob <- matrix(0, nrow = F, ncol = length(pat))
  sum <- matrix(0, nrow = rel.D, ncol =1)
  ...
  for ( j in 1:length(pat)){
     ...
     for ( k in 1:F){
         prob[k,j] <-  dMVN(as.vector(t(mY)), mean = data.mu[k,1:rel.D],data.S[k,1:rel.D,1:rel.D], log= TRUE)
       }
  
    sum[,1] <- sum[,1] +  w[pat[j]] * as.vector(data.mu[which.max(prob[,j]),1:rel.D]) +  (1-w[pat[j]])* as.vector(t(mY))
  }  
  Y.rel[i,] <- t(c(as.vector(sum[,1]/length(pat)))) 
  
}



###### The actual Class labels for the Patients
z.true <- kmeans(Y.rel, centers =Z, nstart =10)$cluster


### A little Visualization of the X Data ##############
pc <- prcomp(Y.rel)
pc.pred <- predict(pc,newdata = Y.rel)
plot(pc.pred[,1], pc.pred[,2], pch = 19,col = z.true)
...
for ( i in 1:Z){
  Y.rel.sc.list[[i]] <- scale(Y.rel[z.true == i,1:D], center = TRUE, scale = TRUE)
}
...
for ( i in 1:Z){
  time.noise.list[[i]] <- rnorm(sum((z.true ==i)+0 ), mean = 5*i, sd = i)
}
