posteriortimeparameters = function(c, That, lambda2,tau2,sigma2,beta0, betahat, Y, K, r, si, Time, N, sig2.data ) {
  ...
  for (j in 1:length(activeclass)) {
    ...
    {
      tau2[activeclass[j],] <- priordraw(r, si,N,D, sig2.dat)$tau2
    }
    
    betahat[activeclass[j],] <- as.vector(priordraw(r, si,N,D, sig2.dat)$betahat)
